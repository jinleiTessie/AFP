...
class Simulator(object):
    ...
    def step(self, action):
        ...
        if (action == 'buy'):
            # long A short B
            A_cost = position_A*self.prices.ix[self.date, self.stock_A]
            B_cost = position_B* self.prices.ix[self.date, self.stock_B]
            net_cost = A_cost- B_cost
            self.costs.append(net_cost)
            self.cum_return.append(net_cost/self.costs[0])
        ...
        self.data_out.append(self.date.isoformat()[0:10] + ',' +str(self.cum_return[-1]))
        print ('for this step data out',self.data_out )
        self.dateIdx += 1
