...
#        print (states)
    ...
    def step(self, action):
        ...
        self.data_out.append(self.date.isoformat()[0:10] + ',' + str(self.portfolio['cash']) + ',' + str(self.prices.ix[self.date, self.stock_A]) + ',' + str(self.prices.ix[self.date, self.stock_B])  + ',' + action + ',' + str(abs_return_A) + ',' +  str(pct_return_A) + ',' + str(abs_return_B) + ',' + str(pct_return_B) + ',' + str(self.prices_ibovespa.loc[self.date]) + ',' + str(self.cum_reward/self.dateIdx) + ',' + str(self.port_val) + ',' + str(self.port_val_market) )
...
#        del df
    ...
    def get_state(self, date):
        ...
        if (date == self.prices.index[-1]):
            ...
            file.write('date,cash,A_price,B_price,action,abs_return_A,pct_return_A,abs_return_B,pct_return_B,prices_ibovespa,cum_reward,port_val,port_val_market')
            file.write('\n')
...
#            self.plot_res(train_df.copy(), 'training')
    ...
    #        df[['port_val']].plot(title='port_val', ax=ax2)
            ...
            act_A = train_df[['action']].replace('hold',0).replace('buyA',1).replace('sellA',-1).replace('buyB',0).replace('sellB',0)
    ...
    #        df[['port_val']].plot(title='port_val', ax=ax2)
            ...
            act_A = test_df[['action']].replace('hold',0).replace('buyA',1).replace('sellA',-1).replace('buyB',0).replace('sellB',0)
            act_B = test_df[['action']].replace('hold',0).replace('buyA',0).replace('sellA',0).replace('buyB',1).replace('sellB',-1)
            acts=pd.concat([act_A, act_B], axis=1)
            acts.columns=['A','B']
            acts.plot(title='actions', ax=ax5)
    ...
    def get_boundary(self):
        ...
        if (self.portfolio['longA']<0)*(self.buy_volume - a_vol[-1] * self.prices.ix[self.date, self.stock_A]) + (self.portfolio['longA']+1<0)*(self.buy_volume - a_vol[-2] * self.prices.ix[self.date, self.stock_A]) + (self.portfolio['longB']>0) * (b_vol[-1] * self.prices.ix[self.date, self.stock_B]-self.buy_volume)  < min_return:
            boundary[0] = 0

        if (self.portfolio['longA']>0)*(a_vol[-1] * self.prices.ix[self.date, self.stock_A]-self.buy_volume) + (self.portfolio['longA']-1>0)*(a_vol[-2] * self.prices.ix[self.date, self.stock_A]-self.buy_volume) + (self.portfolio['longB']<0)*(self.buy_volume - b_vol[-1] * self.prices.ix[self.date, self.stock_B])  < min_return:
            boundary[1] = 0

        if (self.portfolio['longB']<0)*(self.buy_volume - b_vol[-1] * self.prices.ix[self.date, self.stock_B]) + (self.portfolio['longB']+1<0)*(self.buy_volume - b_vol[-2] * self.prices.ix[self.date, self.stock_B]) + (self.portfolio['longA']>0) * (a_vol[-1] * self.prices.ix[self.date, self.stock_A]-self.buy_volume) < min_return:
            boundary[2] = 0

        if (self.portfolio['longB']>0)*(b_vol[-1] * self.prices.ix[self.date, self.stock_B]-self.buy_volume) + (self.portfolio['longB']-1>0)*(b_vol[-2] * self.prices.ix[self.date, self.stock_B]-self.buy_volume) + (self.portfolio['longA']<0)*(self.buy_volume - a_vol[-1] * self.prices.ix[self.date, self.stock_A])  < min_return:
            boundary[3] = 0
